...
- [x] Rate limiting — handled by DigitalOcean App Platform / load balancer
- [x] Enforce session token validation (enforced)
...
- [x] Structured logging with levels — request logging middleware, startup/shutdown logging
- [x] Health check endpoint for load balancer / monitoring (`GET /health`)
- [ ] Database migrations strategy (currently schema is created on startup)
- [x] Graceful shutdown handling — lifespan logs clean startup/shutdown
- [x] Configuration via environment variables (`DNA_` prefix via pydantic-settings)
- [x] CORS configuration — handled by DigitalOcean proxy / CDN; app-level CORS via `DNA_ALLOWED_ORIGINS`
- [x] API documentation (OpenAPI/Swagger auto-generated by FastAPI)
- [x] Error response standardization — global exception handler, validation error formatter
...
- [x] Input validation tightening (character_name max_length, to_agents max_length)
- [x] TLS termination — handled by DigitalOcean load balancer
